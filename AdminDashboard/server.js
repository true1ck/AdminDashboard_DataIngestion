...
app.post('/api/save_local_file', async (req, res) => {
    try {
        ...
        res.json({ success: true, path: targetFile, filename: filename, folder: folder });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});

// Read a local file from DataCollected
app.get('/api/view_local_file', (req, res) => {
    try {
        ...
        const targetFile = path.resolve(__dirname, '..', 'DataCollected', folder, filename);
        
        // Security check: ensure path is within DataCollected
        const dataCollectedRoot = path.resolve(__dirname, '..', 'DataCollected');
        if (!targetFile.startsWith(dataCollectedRoot)) {
...
async function runImport(jobId, config) {
    ...
    try {
        ...
        const outputFilename = `${safeName}_${jobId}.txt`;
        const outputPath = path.join(DATA_COLLECTED_DIR, outputFilename);
        const fileStream = fs.createWriteStream(outputPath);
        
        state.outputFile = { folder: 'database', filename: outputFilename };
        ...
        state.status = 'done';
        await db.updateJob(jobId, { status: 'done', progress: 100, itemsProcessed: totalRows, extraMeta: JSON.stringify(state.outputFile) });
